...
// Load the faucet key.
let baseKeyPath =
  '/Users/Frank/mygit/casper-node/utils/nctl/assets/net-1/faucet/';
...
async function getAccount(publicKey) {
  ...
  let stateRootHash = (await c.getLatestBlockInfo()).block.header
    .state_root_hash;
  let account = await c
    .getBlockState(
      stateRootHash,
...
function setAll(
  ...
  let accounts = accountWeights.map((x) =>
    CLTypedAndToBytesHelper.bytes(x.publicKey.toAccountHash())
  );
